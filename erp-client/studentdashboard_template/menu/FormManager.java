...
public class FormManager {
    ...
    public static void updateTempFormUI() {
        // Update UI delegates for each open form and then call the
        // form's refresh hook so any custom theme-aware repaints or
        // style re-applies happen immediately. This ensures components
        // that rely on custom painting (not just UI delegates) update
        // when themes change.
        for (SimpleForm f : instance.forms) {
            ...
            try {
                ...
                // Defensive: don't let a form's refresh break the theme switch
                ex.printStackTrace();
            }
            f.revalidate();
            f.repaint();
