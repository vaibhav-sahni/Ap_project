package edu.univ.erp.ui.instructordashboard.menu;
...
import edu.univ.erp.ui.instructordashboard.forms.*;
...
public class MyDrawerBuilder extends SimpleDrawerBuilder {
    ...
    public SimpleMenuOption getSimpleMenuOption() {
        ...
        // Dynamically build menu with each assigned section as a separate item
        java.util.List<MenuItem> itemList = new java.util.ArrayList<>();
        itemList.add(new Item.Label("MAIN"));
        itemList.add(new Item("Dashboard", "dashboard.svg"));
        itemList.add(new Item.Label("COURSES"));

        // Fetch instructor's assigned sections and add one menu item per section
        java.util.List<edu.univ.erp.domain.Section> assignedSections = new java.util.ArrayList<>();
        try {
            edu.univ.erp.domain.UserAuth cu = edu.univ.erp.ClientContext.getCurrentUser();
            if (cu != null && "Instructor".equals(cu.getRole())) {
                edu.univ.erp.ui.actions.InstructorActions actions = new edu.univ.erp.ui.actions.InstructorActions();
                assignedSections = actions.getAssignedSections(cu.getUserId());
            }
        } catch (Exception ex) {
            System.err.println("CLIENT WARN: Failed to fetch assigned sections for menu: " + ex.getMessage());
        }

        // Add menu items for each section
        if (assignedSections != null && !assignedSections.isEmpty()) {
            for (edu.univ.erp.domain.Section sec : assignedSections) {
                String menuLabel = sec.getCourseCode() + " - Sec " + sec.getSectionId();
                itemList.add(new Item(menuLabel, "book.svg"));
            }
        } else {
            // Fallback: show a generic "My Courses" item if no sections are assigned
        ...
        simpleMenuOption.addMenuEvent(new MenuEvent() {
            ...
            public void selected(MenuAction action, int[] index) {
                ...
                // Map Dashboard and My Courses
                // Fetch assigned sections again to match the clicked menu item
                if (detectedLabel != null && !detectedLabel.equalsIgnoreCase("Dashboard") && !detectedLabel.equalsIgnoreCase("Logout")) {
                    // Try to match the label to a section
                    try {
                        edu.univ.erp.domain.UserAuth cu = edu.univ.erp.ClientContext.getCurrentUser();
                        if (cu != null && "Instructor".equals(cu.getRole())) {
                            edu.univ.erp.ui.actions.InstructorActions actions = new edu.univ.erp.ui.actions.InstructorActions();
                            java.util.List<edu.univ.erp.domain.Section> sections = actions.getAssignedSections(cu.getUserId());

                            // Match the menu label to a section
                            edu.univ.erp.domain.Section matchedSection = null;
                            for (edu.univ.erp.domain.Section sec : sections) {
                                String expectedLabel = sec.getCourseCode() + " - Sec " + sec.getSectionId();
                                if (detectedLabel.equals(expectedLabel)) {
                                    matchedSection = sec;
                                    break;
                                }
                            }
                            ...
                            if (matchedSection != null) {
                                FormManager.showForm(new edu.univ.erp.ui.instructordashboard.forms.MyCourseForm(matchedSection));
                                return;
                            }
                        }
                    } catch (Exception ex) {
                        System.err.println("CLIENT WARN: Failed to match menu selection to section: " + ex.getMessage());
                    }

                    // Fallback: if no section matched, treat as generic "My Courses"
                    if (detectedLabel.equalsIgnoreCase("My Courses")) {
                        FormManager.showForm(new edu.univ.erp.ui.instructordashboard.forms.MyCourseForm(null));
                        return;
                    }
                }
                ...
                if (index != null && index.length == 1 && index[0] == 0) {
                    FormManager.showForm(new edu.univ.erp.ui.instructordashboard.forms.DashboardForm());
                    return;
                }
                ...
                if (index != null && index.length == 2) {
                    ...
                    if (topLabel != null && topLabel.equalsIgnoreCase("Course Catalog")) {
                        // Route both submenu items to Dashboard (trimmed scope)
                        FormManager.showForm(new edu.univ.erp.ui.instructordashboard.forms.DashboardForm());
                        return;
                    }

                    // legacy email mapping removed: InboxForm and ReadForm were deleted.
                }
            }
        });
